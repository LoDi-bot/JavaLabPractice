...
public class LessonsRepositoryJdbcTemplateImpl implements LessonsRepository {
    ...
    private final ResultSetExtractor<List<Lesson>> lessonsWithCourseAndStudentsResultSetExtractor = rows -> {
    ...
    public List<Lesson> findAll() {
        return jdbcTemplate.query(SQL_SELECT_ALL, lessonsWithCourseAndStudentsResultSetExtractor);
    }

    @Override
    public Optional<Lesson> findById(Integer id) {
        List<Lesson> lessons = jdbcTemplate.query(SQL_SELECT_BY_ID, lessonsWithCourseAndStudentsResultSetExtractor, id);
        if (lessons.size() == 1) {
    ...
    public List<Lesson> findAllByName(String name) {
        return jdbcTemplate.query(SQL_SELECT_ALL_BY_LESSON_NAME, lessonsWithCourseAndStudentsResultSetExtractor, name);
    }

    @Override
    public void saveLessonReferencedToExistingCourse(Lesson lesson) {
    ...
    @Override
    public void updateLessonWithReferencedCourse(Lesson lesson) {
    ...
    @Override
    public void updateLessonWithoutReferencedCourse(Lesson lesson) {
        jdbcTemplate.update(SQL_UPDATE_BY_ID, lesson.getName(), lesson.getDay_of_week(), lesson.getTime(), lesson.getId());
    }

    @Override
    public void deleteLessonWithoutReferencedCourse(Lesson lesson) {
